...
import {
  noteContent, noteId, notePermission, noteTags, permalink, teamPath,
} from '../../flags'
...
export default class Update extends HackMDCommand {
  ...
  async run() {
    ...
    const {content, noteId, permalink, readPermission, tags, teamPath, writePermission} = flags
    ...
    const payload: Parameters<Awaited<ReturnType<typeof this.getAPIClient>>['updateTeamNote']>[2] & {tags?: string[]} = {}
    ...
    try {
      ...
      await APIClient.updateTeamNote(teamPath, noteId, payload as Parameters<Awaited<ReturnType<typeof this.getAPIClient>>['updateTeamNote']>[2])
    } catch (error) {
